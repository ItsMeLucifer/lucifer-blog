...
const PORT = process.env.PORT || 17995;
...
app.models.user.find((err, result) => {
  if (result.length == 0) {
    ...
    app.models.user.create(user, (err, result) => (
      console.log('tried to create user', user, result)
    ));
  }
})
          ...
          app.models.user.findOne((usererr, user) => {
            if (!usererr && user) {
              result.principals.create({
                ...
                console.log('Created principle', err3, principal);
              })
            }
          })
        }
  ...
  if (!err && roles) {
    if (roles.length === 0) {
      app.models.Role.create({
        ...
        console.log('Created?', err2, result);
      });
    }
  }
});